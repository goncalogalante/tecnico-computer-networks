...

{
	...
	switch(msg_received(msg))
	{
		...
		close(new_node->ext_node->listen_udp_fd);
		new_node->ext_node->listen_udp_fd = 0;
    ...
    int pos;
	char output_string[strlen(buffer) + 1];

	int cntr = 0;

    while (ptr != NULL) {
        pos = ptr - buffer;
        printf("O caractere de nova linha está na posição %d.\n", pos);
        ptr = strchr(ptr + 1, '\n');
	...
	for (int m= jj+1; m < kk; m++)
	{
		for(int l = 0; l<100; l++)
		{
		strcpy(nodes[l],buffer[m]);
		break;
		}
	}*/
